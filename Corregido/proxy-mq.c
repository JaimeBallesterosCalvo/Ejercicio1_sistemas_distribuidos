...
int set_value(int key, char *value1, int N_value2, double *V_value2, struct Coord coord) {
    int ret_value;
    // Comprobar longitud de value1 (máximo 255 caracteres + '\0')
    if (strlen(value1) >= 256) {
        fprintf(stderr, "Error: value1 excede los 255 caracteres.\n");
        return -1;  // Error local (no se envía al servidor)
    }


    // Comprobar rango de N_value2 (1 a 32)
    if (N_value2 < 1 || N_value2 > 32) {
        fprintf(stderr, "Error: N_value2 debe estar entre 1 y 32.\n");
        return -1;
    }


    int status = operacion_comun(1, key, value1, &N_value2, V_value2, &coord, &ret_value);


    // Manejar errores de comunicación
    if (status == -1) {
        return -1;  // Error del servidor (ej: clave ya existe)
    } else if (status == -2) {
        fprintf(stderr, "Error de comunicación con el servidor.\n");
        return -2;
    }
...
int get_value(int key, char *value1, int *N_value2, double *V_value2, struct Coord *coord) {
    // Verificar que los buffers de salida no sean NULL
    if (value1 == NULL || N_value2 == NULL || V_value2 == NULL || coord == NULL) {
        fprintf(stderr, "Error: buffers de salida no inicializados.\n");
        return -1;
    }


    int ret_value;
    int status = operacion_comun(2, key, value1, N_value2, V_value2, coord, &ret_value);


    if (status == -1) {
        fprintf(stderr, "Error: clave no existe.\n");
        return -1;
    } else if (status == -2) {
        fprintf(stderr, "Error de comunicación con el servidor.\n");
        return -2;
...
int modify_value(int key, char *value1, int N_value2, double *V_value2, struct Coord coord) {
    ...
    // Validar value1 y N_value2 (igual que en set_value)
    if (strlen(value1) >= 256) {
        fprintf(stderr, "Error: value1 excede los 255 caracteres.\n");
        return -1;
    }
    ...
    int status = operacion_comun(3, key, value1, &N_value2, V_value2, &coord, &ret_value);


    if (status == -1) {
        fprintf(stderr, "Error: clave no existe o datos inválidos.\n");
        return -1;
    } else if (status == -2) {
        fprintf(stderr, "Error de comunicación con el servidor.\n");
        return -2;
